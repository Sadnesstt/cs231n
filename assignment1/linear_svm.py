...
def svm_loss_vectorized(W, X, y, reg):
    ...
    delta = np.ones(scores.shape) # fixed margin(not applied to correct class)
    ...
    margin_except_itself[range(num_train),y] = -np.sum(margin_except_itself, axis = 1)

    dW = np.matmul(X.T, margin_except_itself) / num_train
