...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        scores, cache2 = affine_relu_forward(out, W2, b2)
        ...
        grads['W2'] = dw2 + 0.5 * self.reg * 2 * W2
        grads['b1'] = db
        grads['b2'] = db2
        # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
        ############################################################################
...
class FullyConnectedNet(object):
    ...
    def __init__(
        ...
        for i in range(self.num_layers):
          ...
          self.params['b%d' % (i+1)] = weight_scale * np.random.randn(layer_dims[i+1])
    ...
    def loss(self, X, y=None):
        ...
        for i in range(self.num_layers):
          ...
          if self.use_dropout and i != self.num_layers - 1:
            ...
            caches['dropout%d' % i] = dropout_cache
            #self.dropout_param['seed'] += 1
          
          # store at cache
          caches['layers%d' % (i+1)] = cache
        ...
        for i in reversed(range(self.num_layers)):
          ...
          # parameter setting
          cache = caches["layers%d" % (i+1)]
          w = self.params["W%d" % (i+1)]

          # # dropout
          if self.use_dropout and not i == self.num_layers-1:
            drop_cache = caches['dropout%d'% i]
          ...
          if i == self.num_layers - 1:
            ...
            dout, dw, db, dgamma, dbeta = affine_norm_relu_backward(dout, cache, self.normalization)
            grads['gamma%d' % (i+1)] = dgamma
            grads['beta%d' % (i+1)] = dbeta
          else:
            dout, dw, db = affine_relu_backward(dout, cache)
              
          # regularization and store
